...
| [0007](0007-test-fixtures-are-assembled-not-stored.md) | Test fixtures are assembled, not stored | The known-bad corpus is built from fragments at runtime, because a well-formed fake credential is indistinguishable from a real one and GitHub push protection correctly rejected the literal version. |
